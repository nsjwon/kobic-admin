...
//			$('.DeleteSelectedBtn').attr("id",operatorId);
			$(".btn.btn_delete").attr("id", operatorId);
			$('#pipeline_con').flowchart('removeClassOperators',"selected");
						...
						if(operatorId.replace("node_","") == node.nodeData.nodeName){
							...
							if(node.parameter.parameterInputSize > 0){
								$.each(node.parameter.parameterInput, function(param_index, param){
									...
									inputStr += 		"<label for='paraValue' class=''>Value</label>";
									inputStr +=			"<input type='text' name='paraValue' id='paraValue' class='' value=" + param.parameterValue + " readonly/>";
									
									// fs_content.css 사용중
									if(param.parameterValueType == 'Folder' || param.parameterValueType == 'File'){
										inputStr +=		"<div class='btn_wrap'><button class='btn btn_line_gray' id='input"+node_index+param_index+"Btn' onclick='getUserGboxRoot(\"input"+node_index+param_index+"\",\""+param.parameterValueType+"\")'>Browser</button></div>";
							...
							if(node.parameter.parameterOutputSize > 0){
								$.each(node.parameter.parameterOutput, function(param_index, param){
									...
									outputStr += 		"<label for='paraValue' class=''>Value</label>";
									outputStr +=		"<input type='text' name='paraValue' id='paraValue' class='' value=" + param.parameterValue + " readonly/>";
									
									if(param.parameterValueType == 'Folder' || param.parameterValueType == 'File'){
										outputStr +=	"<div class='btn_wrap'><button class='btn btn_line_gray' id='output"+node_index+param_index+"Btn' onclick='getUserGboxRoot(\"output"+node_index+param_index+"\",\""+param.parameterValueType+"\")'>Browser</button></div>";
							...
							if(node.parameter.parameterOptionSize > 0){
								$.each(node.parameter.parameterOption, function(param_index, param){
									...
									optionStr += 		"<label for='paraValue' class=''>Value</label>";
									optionStr +=			"<input type='text' name='paraValue' id='paraValue' class='' value=" + param.parameterValue + " readonly/>";
									
									if(param.parameterValueType == 'Folder' || param.parameterValueType == 'File'){
										optionStr +=	"<div class='btn_wrap'><button class='btn btn_line_gray' id='option"+node_index+param_index+"Btn' onclick='getUserGboxRoot(\"option"+node_index+param_index+"\",\""+param.parameterValueType+"\")'>Browser</button></div>";
