...
var isStop = false;	// 정지 버튼 사용 유무
var isStart = false;	// 시작 버튼 사용 유무
...
function initSelectedPipeline() {
	...
	/** 파이프라인 활성화 여부 확인 */
	if(isEnabledPipeline() && !isRefresh && !isStop && !isStart) {
		
		var ppData = copy_pipeline_data ? copy_pipeline_data : pipeline_data;
...
//			$.each(copy_pipeline_data.node, function(node_index, node){
			$.each(ppData.node, function(node_index, node){
...
//		let pipelineModelStr = JSON.stringify(copy_pipeline_data);
		let pipelineModelStr = JSON.stringify(ppData);
