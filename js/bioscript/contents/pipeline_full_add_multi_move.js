...
// 다중 노드 이동 :s
var grid;		// flowchart option
var positionRatio;	// flowchart option
var selectedNodesObj;
var pointerX, pointerY, prevLoc; 
var nodesDraggableOptions = {
    start: function(event, ui) {

        if (ui.helper.hasClass('selected')) selectedNodesObj = $('div.selected');
        else {
			// jquery.flowchart.js에서 사용했던 드래그 이벤트가 이걸로 변경되어서 
			// .selected가 아닌 단일 노드의 이동도 이 드래그 이벤트 사용하게됨.
            selectedNodesObj = $(ui.helper);
        }

		grid = $("#pipeline_con").flowchart("option").grid;
		positionRatio =  $("#pipeline_con").flowchart("getPositionRatio");
        var elementOffset = $($(this).parent().parent()).offset();
        pointerX = (event.pageX - elementOffset.left) / positionRatio - parseInt($(event.target).css('left'), 10);
        pointerY = (event.pageY - elementOffset.top) / positionRatio - parseInt($(event.target).css('top'), 10);
        prevLoc = ui.offset;
    },
    drag: function(event, ui) {
        var element = $($(this).parent().parent());
        var elementOffset = $($(this).parent().parent()).offset();
        ui.position.left = Math.round(((event.pageX - elementOffset.left) / positionRatio - pointerX) / grid) * grid;
        ui.position.top = Math.round(((event.pageY - elementOffset.top) / positionRatio - pointerY) / grid) * grid;

		var operatorId = $(event.target).children("div.flowchart-operator-title.ui-draggable-handle")[0].innerText;
        if (!($("#pipeline_con").flowchart("getDataRef").operators[operatorId]).internal.properties.uncontained) {
            var $this = $(this);
            ui.position.left = Math.min(Math.max(ui.position.left, 0), element.width() - $this.outerWidth());
            ui.position.top = Math.min(Math.max(ui.position.top, 0), element.height() - $this.outerHeight());
        }
		...
		// 드래그 주체 노드 외 다른 노드들을 위한 offset 
		var offsetLeft = parseInt(ui.offset.left - prevLoc.left);
		var offsetTop = parseInt(ui.offset.top-prevLoc.top);
        moveSelectedNodes(operatorId, ui.offset.left, ui.offset.top, offsetLeft, offsetTop);
        prevLoc = ui.offset;
...
function moveSelectedNodes(dragOperatorId, operatorLeft, operatorTop, ol, ot){

    selectedNodesObj.each(function(){
        $this =$(this);
        var position = $this.position();
        var left = dragOperatorId == operatorId ? operatorLeft : position.left + ol;
        var top = dragOperatorId == operatorId ? operatorTop : position.top + ot;
...
function draw_structure2(data,area_width,area_height,minimap_width,minimap_height,modify_yn){ // 데이터, 파이프라인영역 가로길이, 파이프라인영역 세로길이, 미니맵 가로길이, 미니맵 세로길이, 링크수정여부(true,false)
	...
	$('#pipeline_con').flowchart({
		...
		onOperatorSelect: function(operatorId) {		//노드 상단 클릭 이벤트
			console.log('[Event][onOperatorSelect] 노드 상단 클릭 ' + operatorId);	
...
//								inputStr +=		"<div><span>유형</span><span class='parameter_icon parameter_" + param.parameterValueType + "'>" + param.parameterValueType + "</span></div>";
								...
								if(param.parameterValueType == 'Folder' || param.parameterValueType == 'File'){
									...
									inputStr +=					"</table>";
									inputStr +=				"</div>";
									inputStr +=				"<div class='btn_box'><button type='button' class='btn_bace' onclick='selectValue2(\"input"+node_index+param_index+"\",\""+param.parameterType+"\",\""+param.nodeID+"\",\""+param.parameterID+"\");'>" + buttonConfirm +"</button>"+"<button type='button' class='btn btn_gray' style='margin-left:0.8rem;' onclick='closeGBoxBrowser();'>" + "닫기" + "</button>"+"</div>";	
						        	inputStr +=			"</div>";
...
//								outputStr +=	"<div><span>유형</span><span class='parameter_icon parameter_" + param.parameterValueType + "'>" + param.parameterValueType + "</span></div>";
								...
								if(param.parameterValueType == 'Folder' || param.parameterValueType == 'File'){
									...
									outputStr +=				"</table>";
									outputStr +=			"</div>";
									outputStr +=			"<div class='btn_box'><button type='button' class='btn_bace' onclick='selectValue2(\"output"+node_index+param_index+"\",\""+param.parameterType+"\",\""+param.nodeID+"\",\""+param.parameterID+"\");'>" + buttonConfirm +"</button>"+"<button type='button' class='btn btn_gray' style='margin-left:0.8rem;' onclick='closeGBoxBrowser();'>" + "닫기" + "</button>"+"</div>";	
						        	outputStr +=		"</div>";
								...
								var downRunStr = "";
								downRunStr += "<button class='btn btn_skyblue btn_check' id='downBtn' onclick='getUserGboxDown(\"down"+"\",\""+param.parameterValue+"\",\""+param.parameterValueType+"\",\""+"Y"+"\")'>결과 확인</button>";	
								...
								downBoxStr +=				"<table id='' class='tableA treegrid_tb'>";
								...
								downBoxStr +=							"<th>" + gboxBrowserCol1 + "</th>";
								downBoxStr +=							"<th>" + gboxBrowserCol2 + "</th>";
								downBoxStr +=							"<th>" + gboxBrowserCol3 + "</th>";
								downBoxStr +=						"</tr>";
								downBoxStr +=					"</thead>";
								...
								downBoxStr +=				"</table>";
								downBoxStr +=			"</div>";
								downBoxStr +=			"<div class='s_txt'>※ Agent 다운로드 : 전용 클라이언트를 이용한 다운로드</div>";
								downBoxStr +=			"<div class='btn_box' style='padding-top: 0px'>";
								downBoxStr +=				"<button type='button' class='btn btn_black btn_down' onclick='fileDownload(\"agent\");'>" + "Agent 다운로드" + "</button>";
								downBoxStr +=				"<button type='button' class='btn btn_black btn_down' style='margin-left:0.8rem;' onclick='fileDownload(\"html5\");'>" + "다운로드" + "</button>";
								downBoxStr +=				"<button type='button' class='btn btn_gray' style='margin-left:0.8rem;' onclick='closeGBoxBrowser();'>" + "닫기" + "</button>"+"</div>";	
								downBoxStr +=			"</div>";
					        	downBoxStr +=		"</div>";
...
//								optionStr +=	"<div><span>유형</span><span class='parameter_icon parameter_" + param.parameterValueType + "'>" + param.parameterValueTypeStr + "</span></div>";
								optionStr +=	"<div><span>유형</span><span class='parameter_icon parameter_" + param.parameterValueType + "'>" + param.parameterValueType + "</span></div>";
								...
								if(param.parameterValueType == 'Folder' || param.parameterValueType == 'File'){
									...
									optionStr +=				"</table>";
									optionStr +=			"</div>";
									optionStr +=			"<div class='btn_box'><button type='button' class='btn_bace' onclick='selectValue2(\"option"+node_index+param_index+"\",\""+param.parameterType+"\",\""+param.nodeID+"\",\""+param.parameterID+"\");'>" + buttonConfirm +"</button>"+"<button type='button' class='btn btn_gray' style='margin-left:0.8rem;' onclick='closeGBoxBrowser();'>" + "닫기" + "</button>"+"</div>";							
						        	optionStr +=		"</div>";
...
//			console.log("[[[ onOperatorUnSelect ]]]         " + operatorId);
...
//				console.log("[[[ onAfterChange ]]]        " + changeType);
