...
//				alertPopup("", messagePipelineActivate, buttonConfirm, buttonClose, false)
				alertPopup("", messageGPipeLineActivate, buttonConfirm, buttonClose, false)
				return ;	
			}										
...
// 모눈종이 누르면 노드 클릭 해제
$(function() {
	$(document).on("click", ".flowchart-links-layer", function(){
...
// 브라우저 종료 또는 새로고침 시 알림창 표출 Event Handler
...
//	$("#pipeline_div").not("#dotLine > *").css("zoom", Number(zoom) + zoomOffset);
////	
////	var dotLine = $("#dotLine");
////	var originX1 = dotLine.getAttribute("x1");
////	var originX2 = dotLine.getAttribute("x2");
////	var originY1 = dotLine.getAttribute("y1");
////	var originY2 = dotLine.getAttribute("y2");
////	
////	
////	//
////	dotLine.setAttribute("x1", originX2);
////	dotLine.setAttribute("y1", originY2);
////	//
//	
...
function onZoom(zoomType) {
	...
	var cx = $("#pipeline_con").width() / 3;
	var cy = $("#pipeline_con").height() / 3;
	
	$("#pipeline_con").panzoom({ contain: 'invert' });
	if ($("#pipeline_con").flowchart("getPositionRatio")) {
	  chartScale = $("#pipeline_con").flowchart("getPositionRatio");
	}

	// zoomType이 in 또는 out 인 경우
	// null 인 경우는 draw_staructure2 를 다시한 경우(저장, 다른 파이프라인 열기 등..)	
	if ( zoomType != null ) {
		// 최소 40%까지만 축소..
		if (chartScale <= 0.5 && zoomType == "out") {
			swal({
				title: "",
				text: "더 이상의 축소는 지원하지 않습니다.",
		...
		var zoomOffset = zoomType == "in" ? 0.1 : -0.1;
		chartScale += zoomOffset;
	}
	
	$("#pipeline_con").flowchart("setPositionRatio", chartScale);
	$("#pipeline_con").panzoom('zoom', chartScale, {
	  animate: false
	});
	
	// 우측, 상단 고정하여 확대/축소 되도록..
	if ( chartScale < 1 ) {
		cx = 0;
		cy = 0;
	}
	$("#pipeline_con").panzoom('pan', cx, cy);

	// 우측 상단 확대/축소 비율 표시 	
	var updateZoom = chartScale;
	updateZoom = Math.floor(updateZoom * 100);
	$("#zoomValue").text(updateZoom + "%");
...
function deleteSelected_link() {
	...
	if ( isEnabledPipeline() && ( getTaskRunStat() == "run" || getTaskRunStat() == "exec" ) ) {
		swal({
			title: "",
			text: messageGPipelineNotEdit,//messagePipelineNotEdit
			confirmButtonText: buttonConfirm
		});
		return;
	}
	...
	if ( linkId != null ) {
		swal({
			title: "",
			text: messageQDelPipelineSelectLink,//messageGDelPipelineSelectLink,	
...
function createPipeline() {
	...
	if ( pipelineName == "" ) {
		err++;
		$("#pipelineNm-error").text(messageGPipelineNameInput);
	...
	if ( pipelineDesc == "" ) {
		err++;
		$("#pipelineDesc-error").text(messageGPipelineInfoInput);
	...
	if ( pipelineName.indexOf(" ") > -1 ) {
		err++;
		$("#pipelineNm-error").text(messageGRemoveSpace);
		$("#pipelineNm-error").removeClass("Dis_none");
	}
	else {
		$("#pipelineNm-error").addClass("Dis_none");
	...
	var regExpFirstLetter = /^[A-Za-z][A-Za-z0-9_\-\.]*$/;
	if (!regExpFirstLetter.test(pipelineName)) {
		err++;
	  	$("#pipelineNm-error").text(messageGIncludFirst);
	  	$("#pipelineNm-error").removeClass("Dis_none");
	}
	...
	if( regExp.test(pipelineName) ){
		err++;
		$("#pipelineNm-error").text(messageGIncludChrct);
		$("#pipelineNm-error").removeClass("Dis_none");		
	}
	
	var regExp2 = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/g;
	if( regExp2.test(pipelineName) ){
		err++;
		$("#pipelineNm-error").text(messageGIncludKorean);
		$("#pipelineNm-error").removeClass("Dis_none");		
	}
...
function refreshWorkspaceList() {
	...
	var clickedWs = $("#clickWorkspaceId").val();
	if ( clickedWs ) {
		var clickedPp = $("#clickPipelineRawId").val();
		if ( clickedPp ) {
			isInstancePipeline = true;	// 공공 파이프라인을 이용한 파이프라인 생성하는 것처럼 	
			$("#pipeModiRawId").val($("#clickPipelineRawId").val());	// 해당 아이디를 선택해 표출할 수 있도록
		}
		showWorkspaceInfo($("#" + $("#clickWorkspaceId").val()), true);
	}
...
//				pipeline_data = data.pipeline_info;
...
//				}
				...
				if ( isInstancePipeline ) {
					// 공공 파이프라인을 분석 활용한 경우 
					if ( $("#instancePipelineId").val() ) selectCheck++;
					// 파이프라인이 생성/수정된 경우 
...
function initSelectedPipeline() {
	...
	if(isEnabledPipeline() && !isRefresh) {
		...
		swal({
			title: "",
			text: messageQWorkingPipeline,	
			showCancelButton: true,
			showCloseButton: true,
			cancelButtonText: buttonNotSave,
			confirmButtonText: buttonSave
		}).then(function(result) {
...
function showSelectedPipeline() {
	...
	if ( currentPipeline ) {
		...
		// 파이프라인 활성화 후 3분마다 저장하기위해 자동 저장 시작 시각 재설정
		enterTime = new Date();
...
//	console.log(currentTask_status);
		// TASK 상태 초기화				 						 
		initRunTaskInfo("save", currentTask_status, currentPipeline.pipelineData.workspaceName, currentPipeline.pipelineData.rawID);
	...
	$.ajax({
		...
		success: function(data) {
			...
			if ( data.subtask_list ) {
				...
				if ( len > 0 ) {
					...
					$.each(data.subtask_list, function(stIndex, subTask) {
						...
						logStr += 	"<div><span>"+nodeCol1+"</span><span>" + subTask.submissionTime + "</span></div>";
						logStr += 	"<div><span>"+nodeCol2+"</span><span class='status_icon2 status_" + subTask.status + "'>" + subTask.status + "</span></div>";
						logStr += 	"<div><button class='btn btn_line_blue open_lp' aria-controls='task-detail' onclick='showSubTaskLogs(\""+ subTask.rawID +"\");'>"+nodeCol3+"</button></div>"; 
						logStr += "</div>";
					});
				}
			}
			...
			if ( isEmpty ) {
				...
				$("h2[name=nodeId]").siblings("div.status-item").children("span").text("wait");
				$("h2[name=nodeId]").siblings("div.status-item").children("span").attr("class", "status_icon2 status_wait");
			}
...
function runSubTask(btn){
	...
	// 실행중인 파이프라인 수정 불가 
	if ( isEnabledPipeline() && getTaskRunStat() == "run" ) {
		swal({
			title: "",
			text: messageGNodeNotActive,
			confirmButtonText: buttonConfirm
		});
		return;
	}
	...
	swal({
		title: "",
		text: messageQExcuteNode,
		showCancelButton: true,
		cancelButtonText: buttonCancel,
		confirmButtonText: buttonExecute
	}).then(function(result) {
		if (result) {
			$.ajax({
				...
				success : function(data) {
					...
					if(data){
						...
						alertMessage = messageAExcuteNode;
...
function stopSubTask(btn){
	...
	swal({
		title: "",
		text: messageQStopNode,
		showCancelButton: true,
		confirmButtonText: buttonStop,
		cancelButtonText: buttonCancel,
	}).then(function(result) {
		if (result) {
			$.ajax({
...
function updateNode(status, nodeId) {
	...
	if ( operatorId != null && operatorId != "" ) {
		...
		$("#r_tab1").find("h2[name=nodeId]").siblings("div.status-item").children("span").text(status);
		$("#r_tab1").find("h2[name=nodeId]").siblings("div.status-item").children("span").attr("class", "status_icon2 status_" + status);
...
function pipeDesign_createWorkspace() {
	...
	if ( !workspaceName ) {
		$("#workspaceName-error").text(messageCreateWorkspace1);
	...
	if ( !workspaceDesc ) {
		$("#workspaceDesc-error").text(messageCreateWorkspace3);
	...
	if ( !workspaceKeyword ) {
		$("#workspaceKeyword-error").text(messageCreateWorkspace4);
	...
	if ( workspaceName.indexOf(" ") > -1 ) {
		$("#workspaceName-error").text(messageGRemoveSpace);
		$("#workspaceName-error").removeClass("Dis_none");
		return;
	}else {
	...
	/** 워크스페이스명 */
	var regExpFirstLetter = /^[A-Za-z][A-Za-z0-9_\-\.]*$/;
	if (!regExpFirstLetter.test(workspaceName)) {
	  	$("#workspaceName-error").text(messageGIncludFirst);
	  	$("#workspaceName-error").removeClass("Dis_none");
	  	return;
	} else {
	  	$("#workspaceName-error").addClass("Dis_none");
	...
	if( regExp.test(workspaceName) ){
		$("#workspaceName-error").text(messageGIncludChrct);
		$("#workspaceName-error").removeClass("Dis_none");
		return;
	}else {
		$("#workspaceName-error").addClass("Dis_none");
	...
	/** 워크스페이스명 */ 
	var regExp2 = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/g;
	if( regExp2.test(workspaceName) ){
		$("#workspaceName-error").text(messageGIncludKorean);
		$("#workspaceName-error").removeClass("Dis_none");
		return;
	}else {
		$("#workspaceName-error").addClass("Dis_none");
				...
				$.ajax({
					...
					success : function(data){
						...
						if(data.result_stat == "STAT_OK"){
							...
							swal({
								title: messageAFCreateWorkspace,
								text: (data.result_msg == 'Workspace name is duplicated.') ? messageGWorkspaceExist : data.result_msg,
								confirmButtonText: buttonConfirm
							}).then(function() {
...
function checkModiWorkspaceValidate() {
	...
	if ( !workspaceName ) {
		$("#workspaceNameUpdate-error").text(messageCreateWorkspace1);
	...
	// WorkspaceName 공백체크 
	if ( workspaceName.indexOf(" ") > -1 ) {
		$("#workspaceNameUpdate-error").text(messageGRemoveSpace);
		$("#workspaceNameUpdate-error").removeClass("Dis_none");
	}
	else {
		$("#workspaceNameUpdate-error").addClass("Dis_none");
...
function pipeDesign_updateWorkspace() {
	...
	var workspaceDesc = $("#workspaceDescUpdate").val().trim();
	if ( !workspaceDesc ) {
		$("#workspaceDescUpdate-error").text(messageCreateWorkspace3);
		$("#workspaceDescUpdate-error").removeClass("Dis_none");
		invalid++;
	}
	...
	if ( !workspaceKeyword ) {
		$("#workspaceKeywordUpdate-error").text(messageCreateWorkspace4);
		$("#workspaceKeywordUpdate-error").removeClass("Dis_none");
		return;		
	}else {
		$("#workspaceKeywordUpdate-error").addClass("Dis_none");
	...
	var regExpFirstLetter = /^[A-Za-z][A-Za-z0-9_\-\.]*$/;
	if (!regExpFirstLetter.test(workspaceName)) {
	  	$("#workspaceNameUpdate-error").text(messageGIncludFirst);
	  	$("#workspaceNameUpdate-error").removeClass("Dis_none");
	  	return;
	} else {
	  	$("#workspaceNameUpdate-error").addClass("Dis_none");
	...
	var regExp = /[ \{\}\[\]\/?,;:|\)*~`!^\+┼<>@\#$%&\'\"\\\(\=]/gi;
	if( regExp.test(workspaceName) ){
		$("#workspaceNameUpdate-error").text(messageGIncludChrct);
		$("#workspaceNameUpdate-error").removeClass("Dis_none");
		return;
	}else {
		$("#workspaceNameUpdate-error").addClass("Dis_none");
	...
	var regExp2 = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/g;
	if( regExp2.test(workspaceName) ){
		$("#workspaceNameUpdate-error").text(messageGIncludKorean);
		$("#workspaceNameUpdate-error").removeClass("Dis_none");
		return;
	}else {
		$("#workspaceNameUpdate-error").addClass("Dis_none");
	...
	swal({
		title: "",
		text: messageQEditWorkspace,
		showCancelButton: true,
		cancelButtonText: buttonCancel,
		confirmButtonText: buttonEdit
	}).then(function(result) {
		if (result) {
...
function pipeDesign_deleteWorkspace() {
	...
	var workspaceRawID = $("#workspaceModiRawId").val().trim();
	var workspaceID = $("#clickWorkspaceId").val().trim();
				...
				$.ajax({
					...
					success : function(data){
						...
						if(data.result_stat == "STAT_OK"){
							...
							var errorMsg = messageAFRemoveWorkspcae;
							// result_msg가 있는 경우 (워크스페이스 내 파이프라인이 존재하여 삭제 불가)
							if ( data.result_msg ) {
								errorMsg = data.result_msg;
							}						
							swal({
								title: "",
								text: errorMsg,//messageAFRemoveWorkspcae,
								confirmButtonText: buttonConfirm
							});
...
function pipeDesign_createPipeline() {
	...
	if(type == 'develop') {
		...
		if ( !isSelectedRootCate || !isSelectedSubCate || !isSelectedPipeline ) {
			swal({
				title: "",
				text: messageGPipelineSeletType,
				confirmButtonText: buttonConfirm
			});
			return;	
		}	
...
function pipeDesign_PipelineValidateCreate() {
	...
	if ( !pipelineName ) {
		$("#pipelineName-error").text(messageCreatePipeline1);
	...
	if ( !pipelineDesc ) {
		$("#pipelineDesc-error").text(messageCreatePipeline5);
		$("#pipelineDesc-error").removeClass("Dis_none");
		invalid++;
	}else {
		$("#pipelineDesc-error").addClass("Dis_none");
	...
	if ( !pipelineKeywords ) {
		$("#pipelineKeywords-error").text(messageCreatePipeline4);
	 ...
	 */
	 /** 파이프라인명 */ 
	if ( pipelineName.indexOf(" ") > -1 ) {		
		$("#pipelineName-error").text(messageGRemoveSpace);
		$("#pipelineName-error").removeClass("Dis_none");
		return false;
	}else {
		$("#pipelineName-error").addClass("Dis_none");
	...
	var regExpFirstLetter = /^[A-Za-z][A-Za-z0-9_\-\.]*$/;
	if (!regExpFirstLetter.test(pipelineName)) {
		$("#pipelineName-error").text(messageGIncludFirst);
		$("#pipelineName-error").removeClass("Dis_none");
		return false;
	} else {
		$("#pipelineName-error").addClass("Dis_none");
	...
	var regExp = /[ \{\}\[\]\/?,;:|\)*~`!^\+┼<>@\#$%&\'\"\\\(\=]/gi;
	if( regExp.test(pipelineName) ){
		$("#pipelineName-error").text(messageGIncludChrct);
		$("#pipelineName-error").removeClass("Dis_none");
		return false;
	}else {
		$("#pipelineName-error").addClass("Dis_none");
	...
	var regExp2 = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/g;
	if( regExp2.test(pipelineName) ){
		$("#pipelineName-error").text(messageGIncludKorean);
		$("#pipelineName-error").removeClass("Dis_none");
		return false;
	}else {
		$("#pipelineName-error").addClass("Dis_none");
...
function pipeDesign_PipelineValidateKeyupUpdate() {
	...
	if ( !pipelineName ) {
		$("#pipelineNameUpdate-error").text(messageCreatePipeline1);
	...
	// pipelineName 공백체크 
	if ( pipelineName.indexOf(" ") > -1 ) {
		$("#pipelineNameUpdate-error").text(messageGRemoveSpace);
		$("#pipelineNameUpdate-error").removeClass("Dis_none");
		return;
	}
	else {
	...
	if ( !pipelineDesc ) {
		$("#pipelineDescUpdate-error").text(messageCreatePipeline5);
	...
	if ( !pipelineKeywords ) {
		$("#pipelineKeywordsUpdate-error").text(messageCreatePipeline4);
	...
	var regExpFirstLetter = /^[A-Za-z][A-Za-z0-9_\-\.]*$/;
	if (!regExpFirstLetter.test(pipelineName)) {
		$("#pipelineNameUpdate-error").text(messageGIncludFirst);
		$("#pipelineNameUpdate-error").removeClass("Dis_none");
		return;
	} else {
		$("#pipelineNameUpdate-error").addClass("Dis_none");
	...
	var regExp = /[ \{\}\[\]\/?,;:|\)*~`!^\+┼<>@\#$%&\'\"\\\(\=]/gi;
	if( regExp.test(pipelineName) ){
		$("#pipelineNameUpdate-error").text(messageGIncludChrct);
		$("#pipelineNameUpdate-error").removeClass("Dis_none");		
		return;
	}else {
		$("#pipelineNameUpdate-error").addClass("Dis_none");
	...
	var regExp2 = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/g;
	if( regExp2.test(pipelineName) ){
		$("#pipelineNameUpdate-error").text(messageGIncludKorean);
		$("#pipelineNameUpdate-error").removeClass("Dis_none");		
		return;
	}else {
		$("#pipelineNameUpdate-error").addClass("Dis_none");
						...
						if(data.result_stat == "STAT_OK"){
							swal({
								...
								if ( data && (data.result_msg != "STAT_ERR")) {
									...
									closePopup();
									$("#pipeline-create").find("#createPsWsName").text("");
									$("#pipeline-create").find("#pipelineName").val("");
									$("#pipeline-create").find("#rootCate").children('option:not(:first)').remove();
									$("#pipeline-create").find("#subCate").children('option:not(:first)').remove();
									$("#pipeline-create").find("#pipelineList").children('option:not(:first)').remove();
									$("#pipeline-create").find("#pipelineVersion").val("");
									$("#pipeline-create").find("#pipelineReference").val("");
									$("#pipeline-create").find("#pipelineKeywords").val("");
									$("#pipeline-create").find("#pipelineDesc").val("");
									getPipelineMainCategoryInExecute();
								}
							});
						}
																...
																// 	
							swal({
								title: messageAFCreatePipeline,
								text: data.result_msg == 'Pipeline name is duplicated.' ? messageGPipelineExist : data.result_msg,
								confirmButtonText: buttonConfirm
							});
						}
					},
...
//									console.log("pipeline-create remove!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 111111111111111111111");
//									getPipelineMainCategoryInExecute();
...
function pipeDesign_updatePipeline() {
	...
	if ( $("#pipelineNavId").val() == pipelineRawId ) {
		...
		var msg = messageQSavePipelineNodeInfo;		
		var isShowCloseButton = true;
		var cancleButtonText = buttonEditBasicInfo;
		var confirmButtonText = buttonSaveNEdit;
		// 실행중인 파이프라인 수정 불가 
		if ( isEnabledPipeline() && ( getTaskRunStat() == "run" || getTaskRunStat() == "exec" ) ) {
			msg = messageQEditPipelineBasicInfo;
			var isShowCloseButton = false;
			var cancleButtonText = buttonCancel;
			var confirmButtonText = buttonEdit;
		}
...
//								console.log(data.result_msg);
							...
							/*
							var errorMsg = messageAFSavePipelineNodeInfo;
							if ( data.result_msg ) {
								errorMsg = data.result_msg;
							}						
							swal({
								title: "",
								text: errorMsg,//messageAFSavePipelineNodeInfo,
								confirmButtonText: buttonConfirm
							});
							*/
							swal({
...
//									console.log(data.result_msg);
	...
	// 현재 활성화된 파이프라인과 수정할 파이프라인이 다른 경우 (origin code)
		...
		swal({
			title: "",
			text: messageAFSavePipelineInfo,
			showCancelButton: true,
			cancelButtonText: buttonCancel,
			confirmButtonText: buttonEdit
		}).then(function(result) {
...
//							console.log(data.result_msg);		
						...
						swal({
							title: "",
							text: (data.result_msg == 'Cannot be deleted while running.') ? messageAFRemovePipelineRunSt : data.result_msg,
							confirmButtonText: buttonConfirm
						});
					}
				},
...
function pipeDesign_runTask() {
	...
	if(getNodeCount() <= 0) {
		alertPopup("", messageGAddNode, buttonConfirm, buttonClose, false)
		return;	
	}
...
//								console.log(data.result_msg);		
							...
							swal({
								title: "",
								text: messageAExecuteAlrPipeline,
								confirmButtonText: buttonConfirm
							}).then(function() {	
...
function pipeDesign_stopTask() {
	...
	/** 노드 정보가 없을 경우 노드 추가 경고창 */
	if(getNodeCount() <= 0) {
		alertPopup("", messageAddNode, buttonConfirm, buttonClose, false)
		return;	
	}
...
function pipeDesign_refreshTask() {
	...
	swal({
		title: "",
		text: messageGPipelineLatestData,
		showCancelButton: true,
		cancelButtonText: buttonCancel,
		confirmButtonText: buttonConfirm
	}).then(function(result) {
...
function pipeDesign_createLink(linkData) {
	...
	/** 링크 커넥터 정보 세팅 */
	linkModel.sourceParamName = linkData.fromConnector;
	linkModel.targetParamName = linkData.toConnector;
...
// 정해진 시간 간격에 따라 자동 저장
function pipeDesign_savePipeline_auto(savedTime) {
	...
	// 실행중인 파이프라인 수정 불가 
	if ( isEnabledPipeline() && ( getTaskRunStat() == "run" || getTaskRunStat() == "exec" ) ) {
		$("#autoSaveMsg").text(messageGAutoSaveStop);	// [한글]
		return;
	}
	...
	$.ajax({
		...
		success : function(data){
			...
			if(data.result_stat == "STAT_OK"){
				// 태스크 상태 -> 저장됨으로 변경
				setTaskStat_save();
//				$("#autoSaveMsg").text( savedTime + "에 자동 저장됨");	// 
				$("#autoSaveMsg").text(messageGAutoSaved.replace('{0}', savedTime));	// [한글]
			}
			else{
				console.log("자동 저장에 실패했습니다.");	// [한글]
//				swal({
//					title: "",
//					text: "자동 저장에 실패했습니다.",
//					confirmButtonText: buttonConfirm
...
function clearPipeline() {
	...
	swal({
		...
		if ( result ) {
			$.each(pipeline_data.node, function(node_index, node){
				if(node.nodeData != null){
					$("#pipeline_con").flowchart('deleteOperator', node.nodeData.nodeName);
				} else {
					$("#pipeline_con").flowchart('deleteOperator', node.nodeData.programName);
				}
			});	
			$('#pipeline_con').flowchart('removeClassOperators',"selected");
			pipeline_data.node = [];
...
function initRunTaskInfo(stat, rStat, workNm, pipeRawId) {
	...
	// 실행 상태에 따라 자동 저장 기능 사용 여부 표시
	if ( rStat == "run" || rStat == "exec" ) {
		$("#autoSaveMsg").text(messageGAutoSaveStop);	// [한글]
	}
	else {
//		$("#autoSaveMsg").text("파이프라인 활성화 후 " + saveTerm/60 + "분 간격으로 자동 저장");	// 
		$("#autoSaveMsg").text(messageGAutoSaveNoti.replace('{0}', saveTerm/60));	// [한글]
	}
}
...
function alertPopup(title, msg, confBtn, cancelBtn, useCancel) {
	...
	msg = utils_isEmptyToDefStr(msg, messageQRun);	// [한글]
	useCancel = utils_isEmptyToDefStr(useCancel, false);	
	confBtn = utils_isEmptyToDefStr(confBtn, buttonConfirm);
	cancelBtn = utils_isEmptyToDefStr(cancelBtn, buttonClose);
