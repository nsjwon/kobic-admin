...
function createPipeline() {
	...
	if ( pipelineName.indexOf(" ") > -1 ) {
		err++;
		$("#pipelineNm-error").text("파이프라인명은 공백을 사용할 수 없습니다.");
		$("#pipelineNm-error").removeClass("Dis_none");
	}
		...
		success: function(data) {
			...
			if (data.page_info.totalCount > 0) {
				...
				$.each(data.workspace_list, function(index, workspace) {
					...
					workspaceList += 		"<span>" + workspace.workspaceName + "(" + workspace.pipelineCount + ")</span>";
...
//	var workspaceId = $(el).attr("id");
	var workspaceId = $(el).children("input").val();
...
//	var minimapStr = "<div class='minimap Dis_none' id='minimapArea'>";
//	minimapStr += 		"<div class='viewport'></div>";
//	minimapStr += 	 "</div>";
//	$("#minimapArea").html(minimapStr);
	...
	if ( isInstancePipeline ) isInstancePipeline = false;
	
//	if ( !isInstancePipeline ) {
////		console.log($("#pipeline_div"));
////		$('#pipeline_div').load(location.href+' #pipeline_div', function() {
////		    $(this).children(':first').unwrap();
////			$('#minimapArea').load(location.href+' #minimapArea', function() {
////			    $(this).children(':first').unwrap();
////				showSelectedPipeline();
////			});
////		});
		...
		success : function(data){
			...
			$.each(pipeline_data.node, function(node_index, node){
				...
				if(node.nodeData.nodeName == linkData.fromOperator){
				    ...
				    /**
					 * 시작 노드의 커넥터 정보 입력
					 * - 전제 1. 시작 노드의 output 파라미터 정보는 무조건 1개 밖에 없음
					 * - 로직 1. 링크 시작 노드의 1개밖에 없는 output 파라미터에 링크의 시작 커넥터 이름(linkData.fromConnector)을 설정 
				     */
				
					pipeline_data.node[node_index].parameter.parameterOutput[0].targetParamName = new Array();
																			
					pipeline_data.node[node_index].parameter.parameterOutput[0].targetParamName[0] = linkData.toConnector.replace(/\|/gi, ":")					
...
function pipeDesign_savePipeline() {
	 ...
	 ***********************************************/
	console.log('::::::::::: pipeDesign_savePipeline ::::::::::::::'); 
