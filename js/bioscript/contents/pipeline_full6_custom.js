...
function draw_structure2(data,area_width,area_height,minimap_width,minimap_height,modify_yn){ // 데이터, 파이프라인영역 가로길이, 파이프라인영역 세로길이, 미니맵 가로길이, 미니맵 세로길이, 링크수정여부(true,false)
	...
	$('#pipeline_con').flowchart({
		...
		onOperatorSelect: function(operatorId) {		//노드 상단 클릭 이벤트			
			...
			custom_onOperatorSelect(operatorId, data);									
		},		
		onOperatorUnselect : function(operatorId) {
			console.log('[Event][onOperatorUnselect] 노드 클릭 해제(draw_structure2)');
			custom_onOperatorUnselect(operatorId);
		},
		onOperatorMouseOver : function(operatorId) {
			// console.log('[Event][onOperatorMouseOver] 노드 위 마우스 위치');
			custom_onOperatorMouseOver(operatorId);
		},
		onOperatorMouseOut : function(operatorId) {
			// console.log('[Event][onOperatorMouseOver] 노드 위 마우스 벗어남');
			custom_onOperatorMouseOut(operatorId);
		},
		onLinkSelect : function(linkId) {
			console.log('[Event][onLinkSelect] 링크 클릭 : linkId : ' + linkId);
			custom_onLinkSelect(linkId);
		},
		onLinkUnselect : function() {
			console.log('[Event][onLinkUnselect] 링크 클릭 해제');
			custom_onLinkUnselect();
		},
//		onOperatorCreate : function(operatorId, operatorData, fullElement) {
//			console.log('[Event][onOperatorCreate] 노드 생성');
//			custom_onOperatorCreate(operatorId, operatorData, fullElement);
//		},
		onLinkCreate : function(linkId, linkData){
			console.log('[Event][onLinkCreate] 링크 생성');
			//custom_onLinkCreate(linkId, linkData);		
		...
		onOperatorDelete : function(operatorId) {
			console.log('[Event][onOperatorDelete] 노드 삭제');
			custom_onOperatorDelete(operatorId);
		},
		onLinkDelete : function(linkId, forced){
			console.log('[Event][onLinkDelete] 링크 삭제');
			custom_onLinkDelete(linkId, forced);			
		},
		onOperatorMoved : function(operatorId, position) {
			console.log('[Event][onOperatorMoved] 노드 이동(draw_structure2)');
			custom_onOperatorMoved(operatorId, position);	
		},			
		onAfterChange : function(changeType){
			console.log('[Event][onAfterChange] 플로우차트 수정 : type : ' + changeType);
		  	custom_onAfterChange(changeType);
		},
		onDblClick : function(operatorId) {
			console.log('[Event][onDblClick] ');
			custom_onDblClick(operatorId);
		}		
    });
...
console.log("click!");
		...
		onOperatorUnselect : function(operatorId) {
			console.log('[Event][onOperatorUnselect] 노드 클릭 해제(draw_structure)');
			$('#pipeline_con').flowchart('removeClassOperator',operatorId,"selected");
		},
		onOperatorMoved : function(operatorId, position){
			console.log('[Event][onOperatorMoved] 노드 이동(draw_structure)');
			custom_onOperatorMoved(operatorId, position);			
		}
...
/** 이벤트 조건 노드 상단 선택 */
function custom_onOperatorSelect(operatorId, data) {
	$('#pipeline_con').flowchart('removeClassOperators',"selected");
			$('#pipeline_con').flowchart('addClassOperator',operatorId,"selected");
...
/** 노드를 선택하지 않을 때 호출 */
function custom_onOperatorUnselect() {
	$('#pipeline_con').flowchart('removeClassOperator',operatorId,"selected");	
}
/** 노드 위에 마우스 위치 */
function custom_onOperatorMouseOver(operatorId) {
	
}
/** 노드 위에 마우스 벗어남 */
function custom_onOperatorMouseOut(operatorId) {
	
}
/** 링크 클릭 */
function custom_onLinkSelect(linkId) {
	
}
function custom_onLinkUnselect() {
	
}
function custom_onOperatorCreate(operatorId, operatorData, fullElement) {
	
}
function custom_onLinkCreate(linkId, linkData) {
		
}
function custom_onOperatorDelete(operatorId) {
	
}
function custom_onLinkDelete(linkId, forced) {
...
function custom_onOperatorMoved(operatorId, position) {
	minimapSet();
}
function custom_onAfterChange(changeType) {
	
}
function custom_onDblClick(operatorId) {
